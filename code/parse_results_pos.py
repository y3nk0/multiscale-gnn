...
dl = ['MPNN', 'LSTM', 'MPNN_LSTM', 'Multi-scale MPNN']

f = open('../results/dep_results_FR_vacFalse_2020-12-27_2021-03-27.csv')
...
for line in lines:
    ...
    if model in res1:
        ...
        if model in dl:
            ...
            res1_l[model].append(float(nu)/94)
            res1_st[model].append(float(st)/94)
    else:
        res1[model] = nu
        if model in dl:
            dl_res1_l[model] = [nu]
            ...
            res1_l[model] = [float(nu)/94]
            res1_st[model] = [float(st)/94]
...
for line in lines:
    ...
    if model in res2:
        ...
        if model in dl:
            ...
            res2_l[model].append(float(nu)/94)
            res2_st[model].append(float(st)/94)
    else:
        res2[model] = nu
        if model in dl:
            dl_res2_l[model] = [nu]
            ...
            res2_l[model] = [float(nu)/94]
            res2_st[model] = [float(st)/94]
...
for r in res2_l:
    ...
    for i, txt in enumerate(res2_l[r]):
        axs[counter].annotate(str(round(txt,1)), (x[i], txt+80), ha='center', fontsize=6)
    axs[counter].set_ylim(0, 300)
...
fig.savefig('../images/results_base.png', bbox_inches='tight', dpi=400)
plt.show()


cols = ['tab:purple', 'tab:brown', 'tab:pink', 'tab:grey', 'tab:olive', 'tab:cyan', 'gold']
## dl
...
fig.text(0.5, 0.02, 'Days', va='center', ha='center')
fig.text(-0.04, 0.5, 'Prediction error', va='center', ha='center', rotation='vertical')
fig.savefig('../images/results.png', bbox_inches='tight', dpi=400)
plt.show()
